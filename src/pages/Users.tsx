...
import { useUsers, useUpdateUser, useDeleteUser, type User } from '@/hooks/useUsers';
...
import { Search, Users as UsersIcon, AlertCircle, Loader2, RefreshCw, Eye, Trash2 } from 'lucide-react';
            ...
            <div className="space-y-6 mt-6">
              ...
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-full bg-dash-purple/10 flex items-center justify-center">
                  <span className="text-dash-purple text-xl font-bold">{selectedUser.fullName?.charAt(0)?.toUpperCase()}</span>
                </div>
                <div>
                  <p className="text-lg font-semibold text-dash-text">{selectedUser.fullName}</p>
                  <p className="text-sm text-dash-muted">{selectedUser.email}</p>
                </div>
              </div>

              {/* Info */}
              <div className="space-y-3 text-sm">
                <div className="flex justify-between"><span className="text-dash-muted">Phone</span><span className="text-dash-text">{selectedUser.phone}</span></div>
                <div className="flex justify-between"><span className="text-dash-muted">Rentals</span><span className="text-dash-text">{selectedUser.nbr_fois_allocation}</span></div>
                <div className="flex justify-between"><span className="text-dash-muted">Outstanding</span><span className="text-dash-text">{selectedUser.facture} TND</span></div>
                <div className="flex justify-between"><span className="text-dash-muted">Joined</span><span className="text-dash-text">{new Date(selectedUser.createdAt).toLocaleDateString()}</span></div>
              </div>
              ...
              <div className="space-y-4 border-t border-dash-border pt-4">
                <div className="flex items-center justify-between">
                  ...
                  <Switch checked={selectedUser.status === 'ACTIVE'} onCheckedChange={() => handleToggleStatus(selectedUser)} className="cursor-pointer" />
                </div>
                <div className="flex items-center justify-between">
                  <div>
                  ...
                  <Switch checked={selectedUser.blacklist} onCheckedChange={() => handleToggleBlacklist(selectedUser)} className="cursor-pointer" />
                </div>
              </div>

              {/* Booking History */}
              <UserBookingHistory userId={selectedUser._id} />
            </div>
          )}
        </SheetContent>
      </Sheet>
...
function UserBookingHistory({ userId }: { userId: string }) {
        ...
        <div className="space-y-2">
          {bookings.map((b) => (
            <div key={b._id} className="flex items-center justify-between p-2.5 rounded-lg bg-dash-bg text-xs">
              <span className="text-dash-text">{new Date(b.startDate).toLocaleDateString()} → {new Date(b.endDate).toLocaleDateString()}</span>
